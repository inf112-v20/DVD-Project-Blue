...
import inf112.RoboRally.app.views.game.GameScreenForTesting;

import static com.badlogic.gdx.graphics.Texture.TextureFilter.Linear;

public class TestGameMenu implements Screen {
    ...
    public TestGameMenu(GameLauncher gameLauncher) {
    ...
    public void show() {
        ...
        START.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                stage.getRoot().addAction(Actions.sequence(Actions.fadeOut(0.1f), Actions.run(() -> gameLauncher.setScreen(new GameScreenForTesting(gameLauncher)))));
            }
        });
        ...
        BACK.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                stage.getRoot().addAction(Actions.sequence(Actions.fadeOut(0.1f), Actions.run(() -> gameLauncher.setScreen(new MainMenu(gameLauncher)))));
            }
        });
