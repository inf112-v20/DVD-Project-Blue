...
public class StepDefinitions implements En {
    ...
    public StepDefinitions() {
        ...
        Given("a robot {string} placed at \\({int}, {int}), facing {word}", (String name, Integer x, Integer y, String facing) -> {
            int robotId = addRobot(name);
            board.placeRobot(robotId, new Position(x, y), Direction.RIGHT);
        });

        Then("the robot {string} moves forward", (String name) -> {
            int robotId = getRobot(name);
            board.moveRobot(robotId, false);
        });

        Then("the robot {string} has position \\({int}, {int})", (String name, Integer x, Integer y) -> {
            int robotId = getRobot(name);
            assertEquals(board.getRobotPosition(robotId), new Position(x, y));
        });
    }
